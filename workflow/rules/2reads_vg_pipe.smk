...
rule 2reads_construct_vg:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/construct_vg_{sample}_{chr}.log"
    shell:
        r"vg construct -C -R {wildcards.chr} -r {input[0]} -v {input[1]} -t 1 -m 32 -a > {output}"

rule 2reads_sorting_DAG:
    input:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/sortingDAG_{sample}.log"
    shell:
        r"""
        vg ids -j {input} && echo "sorting DAG finish" > {output}
        """

rule 2reads_indexing_xg:
    input:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/indexingXG_{sample}.log"
    shell:
        r"""
        ls {input.test};
...
rule 2reads_convert_vg:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/convertVG_{sample}.log"
    shell:
        r"vg convert --gfa-out {input} > {output}"

rule 2reads_giraffe_autoindex:
    input:
...
rule 2reads_giraffe_mapping:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/giraffe_mapping_{sample}.log"
    shell:
        r"""
...
rule 2reads_giraffe_snarls:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/giraffe_snarls_{sample}_{chr}.log"
    shell:
        r"""
        vg snarls {input} > {output} 
        """

rule 2reads_giraffe_chunk:
    input:
        "vg_vg/{sample}_wg.xg",
        "vg_map/{sample}.gam"
    output:
...
rule 2reads_giraffe_pack:
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/giraffe_pack_{sample}_{chr}.log"
    shell:
        r"""
        vg pack -x {input[0]} -g {input[1]} -Q 5 --threads 3 -o {output}
        """

rule 2reads_giraffe_call: 
    input:
        "vg_vg/{sample}_pt{chr}.vg",
    ...
    conda:
        "../envs/vg.yml"
    log:
        stderr="logs/2reads_vg/giraffe_call_{sample}_{chr}.log"
    shell:
        r"""
        vg call {input[0]} -k {input[1]} -r {input[2]} -s {wildcards.sample} --threads 3 > {output} 
        """
    
rule 2reads_giraffe_concat:
    input:
    ...
    log:
        stderr="logs/2reads_vg/giraffe_concat_{sample}.log"
    params:
        tempdir="../results/tmp/"
    shell:
        r"""
...
rule 2reads_giraffe_tabix:
    input:
        "vg_call/wg_{sample}.vcf.gz"
    output:
        "vg_call/wg_{sample}.vcf.gz.tbi"
    log:
        stderr="logs/2reads_vg/giraffe_tabix_{sample}.log"
    params:
        "-p vcf"
    wrapper:
